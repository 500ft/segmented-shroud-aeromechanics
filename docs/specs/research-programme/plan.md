...
- **D9. Acceptance rule.** Default (same owner rule): the most-used validation procedure, not project-specific percentages. **Amended 2026-09-25 to agree with the [uncertainty decision record](uncertainty-decision-record.md); the original wording called this a pass rule and is withdrawn.** Comparison error E = S − D (simulation minus data) is compared with the validation uncertainty U_val = √(U_num² + U_input² + U_D²). Following Eca, Dowding and Roache (`doi:10.1115/1.4053854`), V&V 20 estimates the discrepancy for specified outputs and conditions; it is not inherently a pass or fail exercise. `|E| ≤ U_val` is therefore this project's consistency screen at the stated band, and a wider band must never make a less informative computation look more accurate. U_val exists only when every component is quantified; otherwise the verdict is `INCOMPLETE_UNCERTAINTY` and the size of the partial combination is reported under its own name. Numerical uncertainty U_num by the three-grid GCI procedure of Celik et al. (ASME J. Fluids Eng. 2008), Fs = 1.25, refinement ratio ≥ 1.3, the most-cited grid-convergence method. Experimental uncertainty U_D from the source of record where given. The published experiment-to-CFD spread for each case (the TMR's own model comparisons for A0.1; the many Caradonna–Tung CFD reproductions for A0.2) is recorded alongside as the reporting context, so the reader sees where this toolchain sits in the field, and it is not the pass rule. Percentages appear only as reported numbers.
- **D10. A0.2 solver regime — OPEN, decide before A0.2 meshing. Restated 2026-09-25.** Source verification established that the 1250 rpm baseline sits at **Mtip = 0.437**, not an incompressible condition. The Prandtl–Glauert factor there is 1.11; the earlier wording turned that into a predicted 11% influence on outboard sectional Cp, and [that reading is withdrawn](a0-validation-cases.md). It is a sensitivity flag, not a quantified correction for a three-dimensional rotor with induced flow, so **option (b) can no longer be read as a bias of known size**.

  Options: (a) a verified compressible workflow for A0.2; (b) incompressible, gating only integrated thrust coefficient and reporting sectional Cp with a compressibility bias that is declared as **unquantified**; (c) incompressible, comparing Cp only at inboard stations where the influence is smaller, with that claim boundary stated.

  **Recommendation for a pressure-comparison claim: (a).** If sectional Cp is to support any claim, the regime that omits the physics cannot be the one used, and no linearised factor will repair it afterwards. Its cost is setup and solver time, and it addresses omitted compressibility only; it does not remove turbulence-model or geometry bias. If the decision is that only integrated thrust matters, (b) is coherent provided the Cp bias is reported as unquantified rather than as 11%. **This is a recommendation to the decision owner and is not adopted here.** D10 still gates the A0.2 acceptance file, which cannot be frozen until it is settled.

  D10 does **not** gate the A0.1 fine-grid SST recovery, which is a separate work item with its own prerequisites.
