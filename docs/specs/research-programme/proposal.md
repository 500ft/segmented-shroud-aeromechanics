...
- **Rule (amended 2026-09-25).** CFD does not set the minimum effect of interest; it bounds it. The owner sets **IN-03**, the minimum meaningful power difference at matched thrust in W (or a declared ratio with its denominator), as a decision: the smallest effect that would change a design choice, informed by Study A's range.

  **IN-01 is a separate decision, not a conversion of IN-03.** The earlier rule set IN-01 = IN-03 ÷ dP/dc̄ using a sensitivity taken from the large computational rotor. That inversion is withdrawn. Power detectability in W and clearance-setting fidelity in µm are two requirements about two different things, and they may be linked only through a model that is actually supported. For any such conversion to mean anything it needs all of: the same machine, the same endpoint and the same operating point on both sides; a local slope that is justified where it is evaluated, with its sign, units and uncertainty stated; and a rule for what happens when the slope approaches zero or changes sign, where the inversion is unstable or meaningless. A scalar uniform-clearance derivative also cannot stand in for the influence of every seam descriptor, which is the thing Study A exists to characterise.

  Until such a model exists for the actual apparatus, IN-01 is allocated from an independently chosen engineering target and recorded as an owner decision in its own right. **IN-02**, target mean clearance, follows from the A0 baseline geometry.
...
- **Feasibility assessment (amended 2026-09-25; the former kill criterion is withdrawn).** The earlier rule killed Study C whenever the required combined uncertainty fell below about half of one published accuracy figure (25 µm) for one sensor class. That threshold is withdrawn. An accuracy quoted for a different rig is not an installed standard uncertainty here, and a single sensing paper does not establish the best achievable limit of all methods.

  In its place: name the candidate methods actually under consideration, and for each record an installed calibration and feasibility plan rather than a catalogue number. A method is ruled out by evidence about *this* installation, not by comparison with a literature bound. The published 25 µm figure stays in the budget register as a `literature_bound` that is recorded and never combined, which is already how `scripts/clearance_uncertainty_budget.py` treats it. Contributors that remain unresolved stay pending rather than being decided by a proxy. No procurement is implied by any of this.
