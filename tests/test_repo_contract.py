...
class RepositoryContractTests(unittest.TestCase):
    def test_contract_script_passes(self) -> None:
        completed = subprocess.run([sys.executable, str(ROOT / "scripts/check_repo_contract.py")],
                                   cwd=ROOT, capture_output=True, text=True)
        self.assertEqual(completed.returncode, 0, completed.stdout + completed.stderr)

    def test_example_is_unambiguously_not_a_result(self) -> None:
        example = json.loads((ROOT / "protocols/example-specimen-manifest.json").read_text(encoding="utf-8"))
    ...
    def test_research_dependency_graph_has_no_dangling_endpoints(self) -> None:
        graph = json.loads((ROOT / "docs/research-dependency-graph.json").read_text(encoding="utf-8"))
